...
async def main(path):
    ...
    print(len(empty_glyphs), "empty_glyphs", "".join([chr(int(x[3:],16)) for x in sorted(list(empty_glyphs))]), '\n')

    print("total unicode names", len(fully_made_of_components)+len(mix_contours_components)+len(fully_made_of_contours)+len(empty_glyphs))

    return dict(green_glyphs = green_glyphs,
...
def export_csv_characters_colors_evolution(report, reportDir, project_name):
    evolution_character_glyphs_color_path = reportDir / "resources" / f"{project_name}_evolution_character_glyphs.csv"
    ...
    if not evolution_character_glyphs_color_path.is_file():
        ...
        with open(evolution_character_glyphs_color_path, "r", encoding = "utf-8") as csvfile:
